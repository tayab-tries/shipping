...
export async function publishCmsEntity(
  ...
  try {
    ...
    if (deployHookUrl && deployHookUrl.startsWith('http')) {
      ...
      console.warn('CLOUDFLARE_DEPLOY_HOOK_URL environment variable is not configured.');
      warning = 'Published to CMS DB. Note: CLOUDFLARE_DEPLOY_HOOK_URL environment variable is missing.';
    }
...
export async function restoreCmsRevision(
  ...
  try {
    ...
    // Restoring creates a NEW version snapshot and triggers publication
