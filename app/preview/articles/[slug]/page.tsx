...
export const metadata: Metadata = {
  title: `Preview Article | ${siteConfig.name}`,
  robots: {
    index: false,
    follow: false,
  },
      ...
      <Container>
        <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-700 rounded-md flex items-center justify-between text-amber-900 dark:text-amber-200">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold">Unpublished Article Draft ({slug})</span>
          </div>
          <span className="text-xs font-mono">robots: noindex</span>
        </div>

        <SectionHeading
          badge="Article Draft Preview"
          title={`Guide Draft: ${slug}`}
          subtitle="Preview of article Markdown draft content."
        />

        <div className="mt-8 p-6 bg-surface border border-border rounded-md space-y-4 max-w-3xl">
          <p className="text-body-md text-muted-foreground">
            This preview route renders draft article copy for editorial review prior to static Edge deployment.
          </p>
        </div>
      </Container>
    </div>
